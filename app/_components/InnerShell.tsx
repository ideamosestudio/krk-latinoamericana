...
export function SiteFooter() {
  ...
  return <footer><div className="container footer-top"><div className="footer-brand"><img src={`${base}/images/KRK-LOGO-BLANCO-3.png`} alt="KRK Latinoamericana" /><p>{text.description}</p></div><a href="#top">{text.top}</a></div><div className="container footer-grid"><div><b>{text.offices}</b><p>{text.officeAddress}</p></div><div><b>{text.plant}</b><p>{text.plantAddress}</p></div><div><b>{text.navigation}</b><p><a href={`${base}/quienes-somos/`}>{text.about}</a><br /><a href={`${base}/servicios/`}>{text.services}</a><br /><a href={`${base}/#sectores`}>{text.industries}</a></p></div><div><b>{text.presence}</b><p>{text.countriesLine1}<br />{text.countriesLine2}</p></div></div><div className="container footer-bottom"><span>© 2026 KRK LATINOAMERICANA S.A.</span><span>MOVING WHAT MATTERS</span></div></footer>;
}
