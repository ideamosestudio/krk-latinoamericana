...
export function SiteHeader() {
  return <header className="inner-nav"><nav className="nav-shell"><a href={`${base}/`} className="brand"><img src={`${base}/images/KRK-LOGO-BLANCO-3.png`} alt="KRK Latinoamericana" /></a><div className="nav-links"><a href={`${base}/quienes-somos/`}>Quiénes somos</a><a href={`${base}/servicios/`}>Productos y servicios</a><a href={`${base}/#sectores`}>Industrias</a><a href={`${base}/#experiencia`}>Experiencia</a></div><a className="nav-cta" href={`${base}/#contacto`}><span>Iniciar un proyecto</span><i>↗</i></a></nav></header>;
}
...
export function InnerHero({ eyebrow, title, accent, image }: { eyebrow: string; title: string; accent?: string; image: string }) {
  return <section className="inner-hero" id="top"><img src={`${base}/images/${image}`} alt="" /><div className="inner-hero-shade" /><div className="container inner-hero-content"><div className="eyebrow light"><span /> {eyebrow}</div><h1>{title}{accent && <><br /><em>{accent}</em></>}</h1></div></section>;
}
